        ...
        {Object.keys(bun).length !== 0 && <ConstructorElement type="top"
             ...
             <div className={constructorStyles.footer + ' mt-10'}>
                ...
                <Button disabled={store.order.orderNumberRequest}
                        type="primary" size="medium" htmlType="submit" onClick={() => store.user.auth ? dispatch(getOrderNumber(order)) :navigate(LOGIN_URL)}> {store.order.orderNumberRequest ? 'Обработка заказа' : 'Оформить заказ'}</Button>
