    ...
    return (
        ...
        {Object.keys(bun).length !== 0 && <ConstructorElement type="top"
             ...
             <div className={constructorStyles.footer + ' mt-10'}>
                ...
                {/*@ts-expect-error*/}
                <Button disabled={store.order.orderNumberRequest} type="primary" size="medium" onClick={() => store.user.auth ? dispatch(getOrderNumber(order)) : navigate.replace({pathname: LOGIN_URL})}> {store.order.orderNumberRequest ? 'Обработка заказа' : 'Оформить заказ'}</Button>
